...
// channel.assertExchange('jobber-email-notification', 'direct');
// channel.publish('jobber-email-notification', 'auth-email', 'this is my message');
...
async function consumeAuthEmailMessages(channel: Channel): Promise<void> {
  try{
    ...
    await channel.assertExchange(exchangeName, 'direct'); // To check if exchange exists -- Course content no. 60
    const jobberQueue = await channel.assertQueue(queueName, { durable: true, autoDelete: false })
    await channel.bindQueue(jobberQueue.queue, exchangeName, routingKey) // Routing page between exchange and queue using routing key
